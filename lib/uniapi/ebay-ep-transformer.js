...
function checkFunction(node, sourceName) {
    if(!node || !sourceName) {
        return false;
    }
    if(node.type !== 'CallExpression') {
    ...
    var type = u.get(node, 'callee.type');
    // if(type !== 'MemberExpression') {
    //     return false;
    // }
    type = u.get(node, 'callee.property.type');
    if(type !== 'Identifier') {
        return false;
    }
    var name = u.get(node, 'callee.property.name');
...
function transformFunctionName(node, parent, sourceName, targetName) {
    var hasFunName = checkFunction(node,sourceName);
    if(!hasFunName) {
        return;
    }


    node.callee.property.name = targetName;
    // console.log('---hasFunName:', sourceName, targetName, node);
}
...
function transformAST(ast, options, moduleOptions) {
    ...
    ast = estraverse.replace(ast, {
        ...
        leave: function(node, parent) {
            ...
            transformFunctionName(node,parent, 'hasFactor', 'hasTreatmentsWithFactor');
